...
$().ready(function () {
	...
	// #2 -----------------
		...
		//console.log("osped - local.drawWidth : " + local.drawWidth);
		...
		//	local.traceImage = {left : $("#traceImage").offset().left, top : $("#traceImage").offset().top};
			// load path
			drawROIArea();
		});
...
function drawROIArea() {
	$.ajax({ 
		url: updateUrl, 
		dataType: 'JSON',
		type: 'GET',
		success: function(data) {
			//var roi = data.ROIArea;
			var roi = data.PostProcess.RoadProcessInfo.ROIArea;
			if (roi == null) {
				return;
			}
			for (var i = 0, len = roi.length ; i < len ; i++) {
				if (roi[i].x == 0.0 && roi[i].y == 0.0)
					return;
				var gridxy = getGridxy(roi[i]);
				drawLoadedPath(gridxy);
			}
		}, error: function(err) {
	...
	function getGridxy(data) {
		...
		var gridxy = {};
		var newX =  Math.round((Number(data.x) + imagePosLeft - centerX) / local.gridSize);
		var newY = -Math.round((Number(data.y) + imagePosTop - centerY) / local.gridSize);

		//console.log("data.x = " + data.x + " , data.y = " + data.y);
		/*
		console.log("imagePosLeft = " + imagePosLeft);
		console.log("local.centerX = " + local.centerX);
		console.log("centerX = " + centerX);
		console.log("local.gridSize = " + local.gridSize);
		...
		*/
		gridxy.x = newX;
		gridxy.y = newY;
		gridxy.color = data.color || selectedColor;

		return gridxy;
	}

	function drawLoadedPath(e) {
		...
		updatePathWithColor(local.paths[local.currentPath], e.color);
		updateExport();
	}
}
			...
			for(var j=0; j<local.paths[i].points.length; j++)
			{
				...
				if (p0.type == 1 || p1.type == 1)
				{
					...
					for(var n=1;n<maxInterpolateSteps;n+=1)
					{
						...
						var r1 = interpolate(q0, q1, k); var r2 = interpolate(q1, p1, k);
